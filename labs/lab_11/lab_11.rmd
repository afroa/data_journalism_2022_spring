...
##This is WV filtered by NAICS code
```{r}
wv_county_energy_naics <- wv_ppp_data %>% 
  filter(str_detect(naics_code,"^21")) %>% 
  
wv_county_energy_naics
```
...
##WV population data with geometry
```{r}
wv_population <- get_acs(geography = "county", 
...
wv_population
```
##Clean the population df
```{r}
wv_clean_population <- wv_population %>%
  rename(COUNTY = NAME) %>%
...
wv_clean_population
```
##Join data plus clean NAs and make it per 100k
```{r}
wv_loans_per_100k <- wv_clean_population %>%
  left_join(project_county_name)  
```

Use the magma color palette, and shade on a "log" scale. 

Also produce a table that shows the total amount of loans per 100,000 population for each county, ranked from highest to lowest, along with the total amount of loans and the total number of loans.
