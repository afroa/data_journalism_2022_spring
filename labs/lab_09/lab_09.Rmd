...
```{r}
wv_ppp_april2020_loans <- read_rds("data/lab_09.rds") %>% 
...
geom_text_repel(aes(label = total_loans)) +
  ...
  labs(
    title="West Virginia PPP Loans in April of 2020",
    x = "date",
    y = "loans per date",
...
pop_by_state_acs <- get_acs(geography = "state", 
              variables = c(population = "B01001_001"), year = 2019)
```
```{r}
#Combine the data

state_ppp_by_100k <- pop_by_state_acs %>% 
  left_join(pop_by_state_acs) %>% 
  arrange(desc(total))
state_ppp_by_100k
