...
**Answer**
This is grouping the Maryland PPP data by county, then counting the total loans in each county, arranging them in descending order, and showing the top-10 counties. 
```{r}
...
**Answer**
Nada! Zilch! (An empty gray square)
...
In ggplot, we work with two key concepts called  geometries (abbreviated frequently as geom) and asthetics (abbreviated as aes). 

Geometries are the shape that the data will take; think of line charts, bar charts, scatter plots, histograms, pie charts and other common graphics forms.  
...
**Answer**
I see a gray scale bar chart that is in ascending order
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
I see a horizontal grayscale bar chart with legible names!
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
I don't see a difference between this and the last iteration
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Answer**
A different color scheme and font, possibly!
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**
The same graph as before but now with a clearly legible title and source
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
A data frame of loans arranged by approval date
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
A line graph with a massive spike sometime between "0" and July 2020
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
This is the same graph but with monthly labels that are rather unintellegible 
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
A much more useful graph than above, with the rotated x-axis labels making it easy to see when the data ebbs and flows
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
