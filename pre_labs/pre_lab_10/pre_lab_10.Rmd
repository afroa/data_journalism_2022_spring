...
This creates a new DF, groups the data by county, and counts the loans in the county
...
**Answer** Describe what the code below is doing and the output.
This adds a filter to the above code, so that only counties with more than 6 loans appear in the data.
...
It's the state code and the county code together with no additional characters or spaces
...
I see the MD fips DF with a new column containing the PPP loan count
```{r}
...
Now there are only populated rows
...
**Answer** Describe the changes from the previous map
Now we can see the counties and their populations when we hover over them.
...
Bar graph:
https://datawrapper.dwcdn.net/Ek2bo/1/

Map:
https://datawrapper.dwcdn.net/qUqZi/1/
