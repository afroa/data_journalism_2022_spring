...
**Answer**
It's a 152584-row DF full of Tweets from federal politicians
```{r}
...
**Answer**
[1] "Dog" "dog" "cat" ","  ———it seems to have separated the specified words into unique words only, which includes a comma
```{r}
...
**Answer**
Looks like all unique words used in the tweets
```{r}
...
**Answer**
A bar graph that shows the frequency of unique word usage, which is interesting after it goes through the common words (the/to/and/of/in) to show the usage of "covid"
...
**Answer**
The frequency and % of unique words used in the Tweets, which is more interesting — the top 10 are all COVID-related
```{r}
unique_words %>%
...
**Answer**
Top ten unique word usage and % for 2020 and 2021, and super interesting because while they're both COVID-heavy, it shows when developments enter the lexicon (like vaccines)
```{r}
unique_words_2020 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
Also heavily-COVID centric, but it does give a more contextual view to the trends. But it still considers "——" a word...
```{r}
covid_tweets %>%
  filter(created < '2021-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
It's interesting that "covid 19" jumped to roughly 5% in 2021, and I'd want to know how often that was paired with "vaccine." My hypothesis is that "covid 19 vaccine" is going to be the leading three-word string
```{r}
covid_tweets %>%
  filter(created >= '2021-01-01') %>%
  unnest_tokens(bigram, content, token = "ngrams", n = 2) %>%
...
**Task** Run the following code and describe the results.
**Answer**
It's interesting because I would not immediately be able to distinguish the results as either Democrat or Republican
```{r}
covid_tweets %>%
...
**Task** Run the following code and describe the results.
**Answer**
It is a lot easier to see the partisan slant in this one. COVID stayed at the top, but the shift to 2021 matches the shift in dialogue ("illegal immigrants," "vaccine mandates," etc., and Trump in 2020 but not 2021)
```{r}
covid_tweets %>%
...
**Answer**
It's interesting that "trump" is positive in 2020 and 2021, even though the difference between the two years is huge (6,689)
```{r}
