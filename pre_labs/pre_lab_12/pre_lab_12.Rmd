...
**Answer**
Df with the state's vaccine administration data
```{r}
...
**Answer**
The number of days each county has records for
...
**Answer**
Vaccine doses by day for the NA-counties
...
**Answer**
A ranked df. In the most recent days, vaccinations have been increasing slightly but are still among the lowest-ranked
```{r}
...
**Answer**
First big surge, then dip with supply issues, then the biggest surge when supply met demand. Gradual falloff as demand decreased
```{r fig.width=9}
ggplot() +
...
**Answer**
This shows a ginormous spike in cases at the turn of the year in the DC suburbs, as Omicron arrived
```{r}
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**
This is the lifetime pattern of COVID cases in MD from 2020 to 2022
```{r}
...
**Answer**
This is the geographic concentration of cases in MD in a cloropleth map
```{r}
ggplot() +
