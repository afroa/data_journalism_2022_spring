...
**Answer**
An assortment of files with different file types.
...
**Answer**
This is a collection of hospitals, and the columns include information like their addresses, city, state, zips, phone, open/ closed status, population, and other relevant information.
```{r}
...
**Answer**
A filtered set from the above data.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A list of MD hospitals organized by type.
```{r}
md_hospitals %>%
  group_by(TYPE) %>%
  summarise(
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Similar as the other frames, but filtered to only include GENERAL MEDICAL AND SURGICAL HOSPITALS
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Now we have a glimpse of pig-latin-y numbers and names, including columns for GEOIDs and other unknown identifiers
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Looks like its the multipolygon data for MD counties
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Maryland! With county boundaries
```{r}
md_counties %>%
  ggplot() + 
  geom_sf() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We've combined the scatterplot with the county map
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
This is the same scatterplot and county map, but with the aes set to show density in terms of beds. Most hospitals have less than 500 beds.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Again filtering by MD acute care hospitals
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A tibble that shows how many hospital beds per county in acute care hospitals in MD
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Population data for MD counties
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Same as above, but this time this includes geometry data
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Capitalized county names and hospital beds
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A cleaned up version of the county data!
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Combining the population data with the hospital bed data
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Same as above, but with 0 taking the place of NAs in the data
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Same as above with a beds per/100k column
```{r}
md_beds_per_100k <- md_county_population %>%
  left_join(md_beds_by_county) %>%
  mutate(total_beds = replace_na(total_beds,0)) %>%
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The county map shaded with the beds_per_100k data
```{r}
ggplot() + 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
More legible than above, using the magma aes
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
...
**Answer**
An even more specific description of beds_per_100k
```{r}
ggplot() + 
  geom_sf(data=md_beds_per_100k, aes(fill=beds_per_100k)) + 
  theme_minimal() +
