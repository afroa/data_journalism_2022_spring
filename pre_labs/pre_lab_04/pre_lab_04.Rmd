...
**Answer**
They do have the same column structure, and the same number of rows.
...
#If I have a row in first data frame that matches a row in the second data frame, the include it and exclude all other rows. In this case, if there's a NAICS row in "maryland_ppp" that matches a row in "naics_codes," then combine the two and find the name, naics_code, and the title, then count the number of rows in the data.
...
# with nrow included to match row total
#If I have a row in first data frame that matches a row in the second data frame, the include it and exclude all other rows. In this case, if there's a NAICS row in "maryland_ppp" that matches a row in "naics_codes," then combine the two and find the name, naics_code, and the title, and show me.
...
**Answer**
Error in `join_rows()`:
! Can't join on `x$zip` x `y$zip` because of incompatible types. This is a datatype mismatch!
...
```{r}
#changes the zip data from <dbl> to <chr>
...
**Answer**
No, there re a lot of "NA's"Unanswered." 
...
**Answer**
Yes? I'm not really seeing a big change.
```{r}
