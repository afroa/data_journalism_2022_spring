...
```{r}
#guess_max=10 tells the code to use the first ten rows to tell what the datatype is going to be. This is why there is the parsing issue, because the data may not behave how R thinks it will. Guess_max exists to set a limit on a gargantuan data set. 

#??- So does this limit the data scanned, or the data loaded???
...
**Answer**
There are a scant 87,927 errors. A flesh wound, really.
...
**Answer**:
How many loans are there where franchise_name is NA? 
192959

How many loans are there where the franchise_name is "Subway"?
275	
...
135073, which is the same number as forgiveness_amount
...
**Answer**:
135073, which is the same number.

All loans that have NA in the forgiveness_amount will have <NA> in the forgiveness_date. This aligns with the logic of the data, because if a loan had one but not the other, it would not make sense (how can it be forgiven without a forgiven date, and versa-vice).
...
**Answer**
There were no loans in October 2020, because the first round of the loan program ended by then. The second round begin in 2021.
```{r}
ppp_maryland_loans %>%
...
I am weary of the smallest amount, because it seems that the processing fees alone would be more than $6...the $10 million is on track because it's the maximum loan amount from the first draw. But $6...?
```{r}
...
**Answer**
There are just two loans less than $100.
```{r}
...
**Answer**
They do, surprisingly. GETGFTD LLC	is going to unionize with wages like that...
```{r}
...
**Answer**
1) The column headers are not clean, they're different caps and some have spaces in them
2) The "amount" values are <chr> instead of <dbl>
...
**Answer**
This changed the name to "x1_id"
...
**Answer**
It says "Error in h(simpleError(msg, call)) :", which I think is an error in the attempt to add up the loans. This is because the amounts are in <chr> instead of <dbl>.
...
**Answer**
There are two duplicates of STUART JONES LLC
```{r}
cleaned_arnold_md_loans %>%
  get_dupes()
```
...
**Answer**
It worked! (Not sure what the question is!)
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
There are now only two mispellings of Arnold.
...
**Answer**
Alas, they are all gone. Is this how spellcheck works? 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
