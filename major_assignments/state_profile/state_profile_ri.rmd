...
```{r}
ri_zip_population <- get_acs(geography = "zcta", variables = c(totalpopulation = "B02001_001"), year = 2019) %>% 
rename(zip = GEOID) 
ri_zip_population
```

```{r}
zip_per_capita <- ri_ppp_by_zip  %>%
  left_join(ri_zip_population, by=c("zip")) %>% 
  mutate(per_capita_zip = total_loans_zip/estimate) %>% 
  select(zip, total_loans_zip, estimate, per_capita_zip) %>% 
